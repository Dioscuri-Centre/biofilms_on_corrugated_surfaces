...
pixelsize=0.21
angle=-1.4 # angle by which the image is rotated wrt to the ideal horizontal orientation of the main channel
...
def process(dir_name,fresults):
    global store,mm,builder,output_name

    poss=get_positions_of_wells(fresults)
    print(poss)
    
    stack = reader.load_folder(dir_name,True)
    axes = stack.metadata.getIntendedDimensions().getAxes()
    print(axes)
    axnb = list(stack.stack.getNextIndex(ax) for ax in axes)
    print(axnb)
    ...
    if (no_channels!=1 or no_zs!=1 or no_positions!=1):
        error
    #print(no_channels, no_times,no_zs,no_positions)

            
    print("loading ",no_times," frames")

    #movie,meta=stack.get_substack(P=0,C=0,Z=0)
    movie=[]
    for t in range(0,no_times):
        imgMM_src = stack.get_img(C=0,P=0,T=t,Z=0,unpack=False)
    ...
    for ni in range(0,len(poss)):
        x1,y1=int(poss[ni,0,0])-50,int(poss[ni,0,1])-80
        x2,y2=int(poss[ni,2,0])+50,int(poss[ni,2,1])+80
...
##        path=os.path.join(output_name,"vel_profile"+str(pi)+"_"+str(ni)+".npy")
        with open(output_name, 'ba') as fout:
            np.save(fout,np.array([ni,x1,y1,x2,y2]))
...
for f in os.listdir(input_dir_name):
    f2=os.path.normpath(os.path.join(input_dir_name,f))
    #print(f2," ",os.path.isdir(f2))
    if (os.path.isdir(f2) is True):
        print(f2)
        fresults=os.path.join(f2,"Results.csv")
